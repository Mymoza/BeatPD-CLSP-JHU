...
from keras.layers import Input, Dense, Lambda, Flatten, Reshape, Activation, Dropout, Add, TimeDistributed, Multiply, Conv1D, Conv2D, MaxPooling1D, AveragePooling1D,GlobalMaxPooling1D,GlobalAveragePooling1D
...
class WaveNetClassifier():
  ...
  def residual_block(self, x, i):
    ...
    skip = Conv1D(self.n_filters,1)(z)
    res = Add()([skip,x])
    return res, skip
  ...
  def construct_model(self):    
    ...
    if self.expand_dims == True:
      ...
      x_reshaped = x
    skip_connections = []
    out = Conv1D(self.n_filters, 2, dilation_rate=1, padding='causal')(x_reshaped)
    for i in range(1, self.dilation_depth + 1):
      out, skip = self.residual_block(out,i)
      skip_connections.append(skip)
    out = Add(name='skip_connections1')(skip_connections)
    out = Activation('relu')(out)
    out = MaxPooling1D(50,padding='same')(out)
    skip_connections = []
    out = Conv1D(self.n_filters,2,dilation_rate=1, padding = 'causal')(out)
    for i in range(1, self.dilation_depth + 1):
      out, skip = self.residual_block(out,i)
      skip_connections.append(skip)
    out = Add(name='skip_connections2')(skip_connections)
    out = Activation('relu')(out)
    #out = Conv1D(self.n_filters, 80, strides = 1, padding='same', name='conv_5ms', activation = 'relu')(out) 
    out = MaxPooling1D(60, padding='same', name='downsample_to_200Hz')(out)
    out = Conv1D(self.n_filters,5,padding='same',activation='relu')(out)
    out = GlobalAveragePooling1D()(out)
    out = Dense(self.output_shape)(out)
